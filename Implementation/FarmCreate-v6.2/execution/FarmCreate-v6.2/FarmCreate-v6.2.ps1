...
Function AutoReboot {
        param ($Message = "Server will Auto Reboot",
            $Header = "Warning",
            [int]$Timer = $xmldata.values.AutoRebootTimer.'#text',
            $force = $xmldata.values.AutoRebootForce.'#text'
        )
        if ($timer -lt 0 -or $timer -gt 190000) {
        write-host "Timer value invalid, setting default of 30"
        $Timer = 30;
        }

        if ($xmldata.values.AutoReboot.'#text' -eq $false) {
        write-host "Config set to not Auto Reboot `r`nExitting Script...A reboot is required"
        exit
        }
      
        If ($psISE) {
            $Shell = New-Object -ComObject "WScript.Shell"
            $Button = $Shell.Popup(($Message +" in "+$Timer+" secs"), $Timer, $Header, 1)
            if ($Button -eq "2") {
            write-host "Cancelling...Exitting Script...A reboot is required."
            exit
            }
            else
            {
            write-host "Server is going to reboot"
            if ($force -eq $false) 
                {
                write-host "reboot no force restart-computer"
                } 
                else { 
        ...
        write-host "Server will reboot in $timer seconds...Ctrl-C to stop the script"
...
if ($xmldata.values.JoinFarm.'#text' -eq $true) {
    ...
    if (!$farm -or $farm.Status -ne "Online") {
        ...
        {
        Write-OutPut "Farm joined. Force Upgrade"
...
#Central Admin
if ($xmldata.values.casecure.'#text' -eq $true) {
        ...
        #2022 requiers next line
        New-SPCentralAdministration -Port $centralAdminPort -WindowsAuthProvider "NTLM" -SecureSocketsLayer -UseServerNameIndication -Url $fullurl -HostHeader $cahost
    } else {
...
Add-LocalGroupMember -Group "Administrators" -Member $xmldata.values.AdminUser.'#text'
Add-LocalGroupMember -Group "Administrators" -Member $xmldata.values.FarmUserName.'#text'
}
else
{
$farm = Get-SPFarm
